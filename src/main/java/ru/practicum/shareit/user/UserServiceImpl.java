...
public class UserServiceImpl implements UserService {
    ...
    public User patchUser(User user, Long userId) {
        user.setId(userId);
        User userFromMemory = userStorage.getUsers().stream().filter(us -> us.getId().equals(userId))
    ...
    public User getUser(long userId) {
        return userStorage.getUsers().stream().filter(user -> user.getId().equals(userId)).findAny()
                .orElseThrow(() -> new NotFoundException("Пользователя с таким id нет"));
    }

    @Override
    public void deleteUser(long userId) {
        userStorage.getUsers().stream().filter(user -> user.getId().equals(userId)).findAny()
                .orElseThrow(() -> new NotFoundException("Пользователя с таким id нет"));
